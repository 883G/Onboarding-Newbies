...
1. איזה שליפות יותר קלות בSQL ואיזה יותר קלות לביצוע בNOSQL?

פעולות של SELECT ודברים בסיסיים שלא משנים את הנתונים קלים יותק בSQL, השליפות שלהם קלות יותר כי הפעולות על הדאטה בייס קלות ופשוטות מאוד.
פעולות של UPDATE ושינוי הנתונים קל יותר בNOSQL , הצורה של המאגר נתונים "גמישה" יותר אז הוספה של נתונים היא קלה יותר ואפשר להוסיף נתונים לא רלציונים בלי לעשות עליהם טרנספורמציות לפני להפיכתם לרלציונים.

2. האם event driven architecture זה יותר stream/bach processing ?

הEDA הוא יותר stream processing מאשר bach, כשאנחנו עובדים לפי אירועים שקורים אנחנו רוצים לעתת להם תגובה דיי מהירה ולכן לעבד אותם באופן מהיר. 
לדוגמה ברשת חברתית, כל פעם שהיוזר מדלג על סרטון או עושה לו לייק אנחנו רוצים לעבד את זה ישר כדי להתאים לו את הסרטונים הבאים שנראה לו. צריך תמיד להיות הכי מעודכנים בEDA אז bach processing פחות מתאים.

3. מה זה active-passive בhigh availability ?

   הactive-passive היא גישה כדי למנוע קריסות בה נגיד אם יש שרת מאסטר אז יש גם שרת מאסטר נוסף שהוא "לגיבוי" בו כשהשרת הראשי שעובד רוצה להפסיק או שהוא קורס אז העבודה עוברת לשרת הגיבוי.
   ככה אם השרת הראשי נפל לא צריך לחכות לשרת משני שיחליף אותו (כשעד אז המערכת מושבתת) אלא פשוט להעביר ישירות את כל הבקשות לשרת הגיבוי שהיה לנו.


4. מה זה Parquet?
   הParquet הוא פורמט של apache לשמירת מידע בצורה המבוססת על עמודות שעושה גם דחיסה וקידוד לנתונים. parquet נועד לשימוש על בסיס hadoop אבל ניתן להשתמש בו גם עצמאית.
   הוא שומר את המידע בעמודות בצורה שמקלה הכי הרבה על תהליך הניתוח נתונים. ובשאילתות הוא מגביל את הקריאה רק לשורות נדרשות כדי לחסוך בזמן I/O ולזרז תהליכים
## Wrapping Up
