...
## Answers
1. שאלת הארכיטקטורה והליבה (The Kernel & Space Separation)

השאלה: הסבר מהו תפקיד ה-Kernel בלינוקס, וכיצד ההפרדה בין User Space ל-Kernel Space תורמת ליציבות ואבטחת המערכת?
 ...
 2. שאלת מחזור חיי התהליך ומשאבים (Process Management & Monitoring)

השאלה: תאר את מסלול החיים של תהליך מרגע הרצתו ועד לסיומו. כיצד תזהה תהליך שצורך משאבים חריגים (CPU/Memory), ובאילו כלים תשתמש כדי לנהל או להפסיק אותו?
 ...
 3.  אלת ניהול האחסון והקובץ (Filesystem & Storage Lifecycle)

השאלה: מהו ה-Inode, וכיצד נראה התהליך המלא של חיבור דיסק חדש למערכת (מרגע הזיהוי ועד לעלייה אוטומטית ב-Boot)?
 ...
 4. שאלת הרשת והשירותים (Networking & Daemons)

השאלה: מהו ההבדל בין תהליך רגיל לבין Daemon, וכיצד תבדוק האם שירות רשת מסוים (כמו SSH) רץ, מאזין בפורט הנכון ונגיש למכונות אחרות?
 ...
 5. שאלת ניהול זיכרון וביצועים מתקדמים (Memory & Virtualization Foundations)

השאלה: הסבר מהו ה-Swap, באילו תנאים המערכת תשתמש בו, וכיצד מנגנונים כמו Namespaces ו-Cgroups מאפשרים את קיומם של קונטיינרים (כמו Docker) על גבי ה-Kernel?
