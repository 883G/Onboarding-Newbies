...
ארכיטקטורת האסמבל בנויה מהירכיה של אוסף של שרתים .
בראש הירכיית ה-ensemble ממוקם הleader node שהוא node הבסיס.
כל שאר הנודים האחרים הם למעשה צאצאיו והם קרויים followers ועוקבים אחר הוראות הleader node .
...
כל פעולות הכתיבה שמתבצעות לnode השונים עוברות דרך הleader , והוא דואג לעדכן בכך את הfollowers, ולאחר שמרבית הצמתים אישרו את פעולות הכתיבה, הוא מבצע אותן.
תפקיד הfollowers הוא להיות משוכפלים בהתאם לleader ולבצע בקשות קריאה.

1-ארכיטקטורת מבנה הנתונים באסמבל מורכבת מהירכיה של צמתים הנקראים "znodes" והיא מאורגנת בצורת עץ.
כל znode מאחסן בתוכו נתונים ורשימת Access Control List (רשימה שמגבילה את הגישה לביצוע פעולות מסוימות תחת אותו הזינוד).
הznode מסוגל לאחסן מידע במערך בתים .
כל znode  מכיל בתוכו בנוסף לנתונים עצמם גם מבנה נתונים ששמו stat שבו מאוכסן metadata של אותו הזינוד
המאטה דאטה כולל: רשימת ACL (רשימה שמגבילה את הגישה לביצוע פעולות מסוימות תחת אותו הזינוד), צמתי הצאצאים של אותו הznode וכו...
יש 4 סוגים שונים של znodes.
Persistent znode- זינוד שקיים תמיד, עד לרגע בו הוא ימחק רשמית (הוא לא נמחק לאחר שהסשן נגמר)
Persistent Sequential znode- זינוד שהוא פרסיסטנט שזוקיפר מצוות לו מספר שעולה כל פעם שנוצרים עוד נודים מסוג זה בסיומת השם שלו לאחר יצירתו
 ephemeral znode- זינוד שקיים עד לרגע שבו הסשן מסתיים
 ephemeral Sequential znode -שזוקיפר מצוות לו מספר שעולה כל פעם שנוצרים עוד נודים מסוג זה בסיומת השם שלו לאחר יצירתו Temporary זינוד מסוג  
...
זוקיפר הוא סקילבילי, ניתן להוסיף עוד נודים לאסמבל על מנת שהוא יוכל להתמודד בצורה טובה יותר עם עומסי קריאה .
אך יש גם לקחת בחשבון שכאשר מוסיפים יותר נודים גם מהירות ביצוע פעולות הכתיבה עולה, מכיוון שגדל מספר הנודים שצריך לאשר את ביצוע פעולת הכתיבה ובשל כך גדלה תדירות התעבורה ברשת באסמבל.  
...
split brain-
באמצעות השימוש ב-quorum ניתן להגדיר את כמות הנודים המינימלית באסמבל שצריכה להשתתף בהצבעה על מנת שיהיה אפשר לבצע פעולת כתיבה או בחירת leader znode חדש (לאחר שהוא נופל) באסמבל.
הquorum יכול לאפשר לאסמבל להמשיך לעבוד ולבצע הצבעות באופן תקין גם כאשר יש נודים שנופלים.
...
מה היתרון של 2 כרטיסים?
היתרון הוא שיש הפרדה בין אימות המשתמש עצמו לבין אימות המשתמש עם השירות בו הוא רוצה להשתמש וזה גורם לכך שהתקשורת ברשת תתבצע בצורה מאובטחת יותר.

מה הקשר בין relm לprinciple?
הrealm מכיל בתוכו principles (הלקוחות שיש להם גישה לשירות)

מה זה ticketcashe, ktab?

ֿticketcashe-
זהו שטח אחסון הממוקם לוקאלית אצל מכונת הלקוח באופן מאובטח ובו מאוחסנים כרטיסי הkerberos ומפתחות הסשן של הלקוח באופן זמני.
הטיקט קאש נותן למשתמש את האפשרות לגשת לשירותי רשת מבלי להזין את פרטי ההתחברות עבור כל אחד מהשירותים שוב ושוב.

ktab- 
זה קובץ שממפה בין הprincipels לבין המפתחות המוצפנים הנגזרים מהסיסמאות המשמשות עבור ההתחברות לkdc.


איך מלקוח פונה לקרברוס בשביל ליצור להרוס ולהציג כרטיסים?ֿ
הלקוח יוצר כרטיס באמצעות הרצת פקודת kinit
הלקוח הורס כרטיסים באמצעות השימוש בפקודה kdestroy
הלקוח מציג כרטיסים באמצעות הרצת הפקודה klist

מריצים את הפקודה klist (יש להריץ את הפקודה בbash).

מה זה CLOCK SKEW בהקשר לקרברוס?
